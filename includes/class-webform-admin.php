            ...
            <div class="webform-card webform-list-card">
                ...
                <?php else : ?>
                    ...
                    <?php foreach ($forms as $form) : ?>
                        <?php $edit_url = admin_url('admin.php?page=webform-builder&form_id=' . $form->ID); $schema = (array) get_post_meta($form->ID, '_webform_schema', true); $stage_count = max(1, count($schema)); $shortcode = '[webform id="' . $form->ID . '"]'; ?>
                        <tr data-form-title="<?php echo esc_attr(strtolower($form->post_title . ' ' . $form->ID)); ?>">
                            <td class="column-primary" data-colname="<?php esc_attr_e('Form', 'webform'); ?>"><div class="webform-form-cell"><span class="webform-form-icon"><span class="dashicons dashicons-feedback"></span></span><div class="webform-form-summary"><strong><?php if ($current_view === 'trash') : ?><?php echo esc_html($form->post_title); ?><?php else : ?><a class="row-title" href="<?php echo esc_url($edit_url); ?>"><?php echo esc_html($form->post_title); ?></a><?php endif; ?></strong><small><?php echo esc_html(sprintf(_n('ID %1$d · %2$d stage', 'ID %1$d · %2$d stages', $stage_count, 'webform'), $form->ID, $stage_count)); ?></small>
                                ...
                                <?php else : ?>
                                    <div class="row-actions"><span><a href="<?php echo esc_url($edit_url); ?>"><span class="dashicons dashicons-edit"></span><?php esc_html_e('Edit', 'webform'); ?></a></span><span><a href="<?php echo esc_url(add_query_arg('panel', 'confirmation', $edit_url)); ?>"><span class="dashicons dashicons-admin-generic"></span><?php esc_html_e('Settings', 'webform'); ?></a></span><span><a href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=webform_duplicate_form&form_id=' . $form->ID), 'webform_duplicate_' . $form->ID)); ?>"><span class="dashicons dashicons-admin-page"></span><?php esc_html_e('Duplicate', 'webform'); ?></a></span><span><a href="<?php echo esc_url(wp_nonce_url(add_query_arg('webform_preview', $form->ID, home_url('/')), 'webform_preview_' . $form->ID)); ?>" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-visibility"></span><?php esc_html_e('Preview', 'webform'); ?></a></span><span class="trash"><button type="button" class="button-link-delete webform-delete" data-id="<?php echo esc_attr($form->ID); ?>"><span class="dashicons dashicons-trash"></span><?php esc_html_e('Trash', 'webform'); ?></button></span></div>
                                <?php endif; ?>
                                </div></div>
