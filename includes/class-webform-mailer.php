...
class Webform_Mailer {
    ...
    public function page() {
        ...
        ?>
        <div class="wrap webform-wrap"><div class="webform-page-head"><div><h1><?php esc_html_e('Email Delivery', 'webform'); ?></h1><p><?php esc_html_e('Send Webform and WordPress email through your SMTP provider.', 'webform'); ?></p></div></div>
        ...
        <div class="webform-card webform-mail-card">
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <label class="webform-check"><input type="checkbox" name="enabled" value="1" <?php checked(!empty($settings['enabled'])); ?>> <?php esc_html_e('Enable SMTP email delivery', 'webform'); ?></label>
                <p class="description"><?php esc_html_e('Leave this disabled if another SMTP or transactional-email plugin already controls WordPress email.', 'webform'); ?></p>
                <div class="webform-mail-grid">
                    <label><?php esc_html_e('SMTP host', 'webform'); ?><input type="text" name="host" value="<?php echo esc_attr($settings['host']); ?>" placeholder="smtp.example.com"></label>
                    <label><?php esc_html_e('Port', 'webform'); ?><input type="number" name="port" min="1" max="65535" value="<?php echo esc_attr($settings['port']); ?>"></label>
                    <label><?php esc_html_e('Encryption', 'webform'); ?><select name="encryption"><option value="tls" <?php selected($settings['encryption'], 'tls'); ?>>TLS / STARTTLS</option><option value="ssl" <?php selected($settings['encryption'], 'ssl'); ?>>SSL</option><option value="none" <?php selected($settings['encryption'], 'none'); ?>><?php esc_html_e('None', 'webform'); ?></option></select></label>
                    <label class="webform-check"><?php esc_html_e('Authentication', 'webform'); ?><span><input type="checkbox" name="authentication" value="1" <?php checked(!empty($settings['authentication'])); ?>> <?php esc_html_e('Use SMTP authentication', 'webform'); ?></span></label>
                    <label><?php esc_html_e('Username', 'webform'); ?><input type="text" name="username" value="<?php echo esc_attr($settings['username']); ?>" autocomplete="username"></label>
                    <label><?php esc_html_e('Password', 'webform'); ?><input type="password" name="password" value="" autocomplete="new-password" placeholder="<?php echo $settings['password'] ? esc_attr__('Saved — leave blank to keep', 'webform') : ''; ?>"></label>
                    <label><?php esc_html_e('From email', 'webform'); ?><input type="email" name="from_email" value="<?php echo esc_attr($settings['from_email']); ?>"></label>
                    <label><?php esc_html_e('From name', 'webform'); ?><input type="text" name="from_name" value="<?php echo esc_attr($settings['from_name']); ?>"></label>
                </div>
                <label class="webform-check"><input type="checkbox" name="force_from" value="1" <?php checked(!empty($settings['force_from'])); ?>> <?php esc_html_e('Use this From address for WordPress email', 'webform'); ?></label>
                <button class="button button-primary"><?php esc_html_e('Save email settings', 'webform'); ?></button>
            </form>
        </div>
        <div class="webform-card webform-mail-card"><h2><?php esc_html_e('Send a test email', 'webform'); ?></h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="webform_send_test_email"><?php wp_nonce_field('webform_send_test_email'); ?><label><?php esc_html_e('Recipient', 'webform'); ?><input type="email" name="recipient" required value="<?php echo esc_attr(wp_get_current_user()->user_email); ?>"></label><button class="button"><?php esc_html_e('Send test email', 'webform'); ?></button></form><p class="description"><?php esc_html_e('A successful test means WordPress handed the message to the configured mail server; final inbox delivery still depends on the provider and DNS authentication.', 'webform'); ?></p></div>
        </div>
        <?php
    }
    ...
    public function test() {
        ...
        $success = $recipient && wp_mail($recipient, __('Webform email delivery test', 'webform'), sprintf(__("This test email was sent from %s at %s.", 'webform'), home_url('/'), current_time('mysql')));
        set_transient('webform_mail_notice_' . get_current_user_id(), array('success' => $success, 'message' => $success ? __('Test email was accepted by WordPress for delivery.', 'webform') : __('The test email failed. Verify the server, port, encryption, username, password, and From address.', 'webform')), MINUTE_IN_SECONDS);
